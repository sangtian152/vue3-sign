import { warn as ue, defineComponent as ee, ref as m, watch as U, onMounted as de, onBeforeUnmount as pe, openBlock as B, createElementBlock as H, createElementVNode as L, normalizeClass as ve, createCommentVNode as A } from "vue";
const fe = "@sangtian152/vue3-sign", ge = "sangtian152", me = "0.0.5", he = "lib/vue3-sign.umd.js", be = "lib/vue3-sign.mjs", we = {
  ".": {
...
function Pe(e, o) {
  ...
  const { values: r, required: l, default: c, type: h, validator: u } = e, b = r || u ? (g) => {
    ...
    if (r && (C = [...r, c], w = w || C.includes(g)), u && (w = w || u(g)), !w && C.length > 0) {
      const _ = [...new Set(C)].map((I) => JSON.stringify(I)).join(", ");
      ue(
...
const Re = (e) => $e(
  ...
  setup(e, { expose: o, emit: r }) {
    ...
    de(() => {
      const t = u.value;
      t.style.background = l.bgColor, b.value = t.getContext("2d", { willReadFrequently: !0 }), _(!0), document.onmouseup = () => {
        N();
      };
    ...
    const g = m(!1), w = (t) => {
      ...
      if (n) {
        ...
        const d = b.value;
        d.lineCap = "round", d.lineJoin = "round", d.strokeStyle = l.lineColor, d.lineWidth = l.lineWidth, Y(n, "source-over");
      }
      const { tapmove: s, tapend: a } = D(), i = u.value;
      i.addEventListener(s, M, { passive: !1 }), i.addEventListener(a, q, { passive: !1 });
    }, M = (t) => {
      if (t.cancelable && t.preventDefault(), g.value || !j.value)
        return;
      const n = z(t, u.value);
      n && (Y(n, null), W.value = n.y, V.value = n.x);
    }, q = (t) => {
      if (t.cancelable && t.preventDefault(), g.value)
        return;
      const n = u.value, { tapmove: s, tapend: a } = D();
      n.removeEventListener(s, M), n.removeEventListener(a, q);
    }, Y = ({ x: t, y: n }, s) => {
      const a = b.value;
      ...
      if (i.clearRect(0, 0, a.width, a.height), i.putImageData(d, 0, 0), i.globalCompositeOperation = "source-over", l.crop) {
        const f = oe(d.data), v = document.createElement("canvas"), x = v.getContext("2d", { willReadFrequently: !0 });
      ...
      let t = u.value, n = !0;
      l.beforeDone && typeof l.beforeDone == "function" && (n = l.beforeDone(t)), n && X().then((s) => {
    ...
    }, re = async () => await X(), ie = (t) => {
      const { tapstart: n } = D();
      b.value.globalCompositeOperation = "source-over", t.removeEventListener(n, F);
    }, le = (t) => {
      if (!l.erasable)
        return;
      const { tapstart: n } = D(), s = b.value;
      s.lineCap = "round", s.lineJoin = "round", s.lineWidth = l.eraserRadius * 2, s.globalCompositeOperation = "destination-out", t.addEventListener(n, F, {
        passive: !1
      });
    }, P = m(0), R = m(0), F = (t) => {
      t.preventDefault();
      const n = u.value, { tapmove: s, tapend: a } = D();
      let i, d, p = z(t, n);
      P.value = p.x, R.value = p.y, K(P.value, R.value), n.addEventListener(s, v, { passive: !1 }), n.addEventListener(a, f, { passive: !1 });
      function f() {
        n.removeEventListener(s, v), n.removeEventListener(a, f);
      }
      function v(x) {
        x.preventDefault(), p = z(x, n), p && (i = p.x, d = p.y, K(P.value, R.value, i, d), P.value = i, R.value = d);
      }
    }, K = (t, n, s, a) => {
      const i = b.value;
    ...
    return o({
      ...
      t.erasable ? (B(), H("div", Te, [
        L("span", {
          class: ve(["signfont", "icon-eraser", { active: g.value }]),
